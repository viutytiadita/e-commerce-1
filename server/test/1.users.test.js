...
describe('Test users', function(){
    describe('post register', function(){
        it('should be an object with 201 status code',function(done){
            const data = {email:'tviuty@yahoo.com', password: '12345', username: 'tviuty', role: 'customer'}
        ...
        it('should be an object with 400 status code(is not a valid email)',function(done){
            const data = {email:'tviuty.yahoo', password: '12345', username: 'tviuty', role:'admin' }
            chai.request(app).post('/users/register')
            .send(data)
            .then(function(res){
        ...
        it('should be an object with 400 status code(length too short)',function(done){
            const data = {email:'tviuty@yahoo.com', password: '123', username: 'tviuty', role:'customer'}
            chai.request(app).post('/users/register')
            .send(data)
            .then(function(res){
                expect(res).to.have.status(400)
